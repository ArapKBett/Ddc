...
use solana_client::rpc_config::GetConfirmedSignaturesForAddress2Config; // Correct import
use chrono::TimeZone;
use std::str::FromStr;
...
fn process_transaction(
    ...
    if let Some(meta) = &tx.transaction.meta {
        for pre_balance in meta.pre_token_balances.as_ref().unwrap_or(&vec![]).iter() {
            if pre_balance.owner.as_deref() == Some(wallet_pubkey.to_string().as_str())
                && pre_balance.mint.as_deref() == Some(usdc_mint_pubkey.to_string().as_str())
            {
